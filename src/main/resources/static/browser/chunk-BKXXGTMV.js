...
var selectIsLoading = createSelector(selectAuthState, (state) => state.isLoading);
var selectIsInitialized = createSelector(selectAuthState, (state) => state.isInitialized);
var selectAuthError = createSelector(selectAuthState, (state) => state.error);
var selectUserRoles = createSelector(selectUser, (user) => user?.roles ?? []);
var selectUserEmail = createSelector(selectUser, (user) => user?.email ?? "");
var selectUserId = createSelector(selectUser, (user) => user?.id ?? null);
var selectUserFullName = createSelector(selectUser, (user) => {
  if (!user)
    return "";
...
var selectIsAdmin = createSelector(selectUserRoles, (roles) => roles.includes("ADMIN"));
var selectIsManager = createSelector(selectUserRoles, (roles) => roles.includes("MANAGER") || roles.includes("ADMIN"));
var selectAuthReady = createSelector(selectIsInitialized, selectIsLoading, (initialized, loading) => initialized && !loading);
...
@angular/core/fesm2022/rxjs-interop.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-BKXXGTMV.js.map
